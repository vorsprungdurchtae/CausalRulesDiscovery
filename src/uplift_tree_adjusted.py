...
from log_processing_adjusted import filter_log, process_data
...
def create_uplift_tree(df, r_copy2):
    for rule in r_copy2:
        ...
        features = df_train
        for col in ['NotCancelled', 'Treatment']:
            features = features.drop(col, axis=1)

        # Train uplift tree
        uplift_model = UpliftTreeClassifier(max_depth=5, min_samples_leaf=200, min_samples_treatment=20,
        ...
        uplift_model.fit(features.values, treatment=df_train['Treatment'].values, y=df_train['NotCancelled'].values)
